...
def plot_kmeans():
    ...
    ax.scatter(X[y_kmeans == 1,3], X[y_kmeans == 1,4], s = 100, c = 'red', label = 'Good Form')
    ax.scatter(X[y_kmeans == 2,3], X[y_kmeans == 2,4], s = 100, c = 'blue', label = 'Bad Form')
    ax.scatter(X[y_kmeans == 3,3], X[y_kmeans == 3,4], s = 100, c = 'green', label = 'Peek Form')
    ax.scatter(centroids[:, 3], centroids[:, 4], s = 150, c = 'cyan', label = 'Centroids')
    ax.set_title('Recent Form of Batsmen Based on their Stats', fontsize=20)
    ax.set_xlabel('Runs', fontsize=15)
    ax.set_ylabel('Batting Average', fontsize=15)
    graph.draw()
...
def plot_best_batsmen():
    ...
    canvas.create_text(370, 10, anchor=W, text=best_batsmen, font='bold')
...
def plot_best_keeper():
    ...
    message = ("The best Keeper of the Tournament could possibly be: ")
    canvas_width = 500
    canvas_height = 500
    root = Toplevel()
    root.geometry("700x600")
    ...
    canvas = Canvas(root, width=canvas_width, height=canvas_height)
    canvas.create_text(1, 10, anchor=W, text=message)
    canvas.create_text(360, 10, anchor=W, text=best_keeper, font='bold')
    img = ImageTk.PhotoImage(Image.open("buttler.jpg"))
    canvas.create_image(0, 30, anchor=NW, image=img)
    canvas.image = img
    canvas.pack()
    root.mainloop()
