...
# Feature Scaling as there are some outliers
...
def plot_kmeans():
    ...
    ax.cla()
    ax.grid()        
    ax.scatter(X1[y_kmeans == 0,2], X1[y_kmeans == 0,4], s = 100, c = 'blue', label = 'Good Form')
    ax.scatter(X1[y_kmeans == 1,2], X1[y_kmeans == 1,4], s = 100, c = 'purple', label = 'Average Form')
    ax.scatter(X1[y_kmeans == 2,2], X1[y_kmeans == 2,4], s = 100, c = 'green', label = 'Peek Form')
    ax.scatter(X1[y_kmeans == 3,2], X1[y_kmeans == 3,4], s = 100, c = 'red', label = 'Poor Form')
    ax.scatter(centroids[:, 2], centroids[:, 4], s = 150, c = 'cyan', label = 'Centroids')
    ax.set_title('Recent Form of Bowlers Based on their Stats', fontsize=20)
    ax.set_xlabel('Wickets', fontsize=15)
    ax.set_ylabel('Bowling Average', fontsize=15)
    graph.draw()
...
def plot_best_bowler():
    ...
    message = ("The best Spinners could possibly be: " + best_bowler1 + " and " + best_bowler2)
    canvas_width = 500
    canvas_height = 500
    root = Toplevel()
    ...
    img = ImageTk.PhotoImage(Image.open("rashid-kuldeep.jpg"))
...
def plot_best_pacer():
    ...
    message = ("The best Pacer of the Tournament could possibly be: ")
    canvas_width = 500
    canvas_height = 500
    root = Toplevel()
    root.geometry("700x600")
    ...
    canvas = Canvas(root, width=canvas_width, height=canvas_height)
    canvas.create_text(1, 10, anchor=W, text=message)
    canvas.create_text(350, 10, anchor=W, text=best_pacer, font='bold')
    img = ImageTk.PhotoImage(Image.open("cummins.jpg"))
    canvas.create_image(0, 20, anchor=NW, image=img)
    canvas.image = img
    canvas.pack()
    root.mainloop()
